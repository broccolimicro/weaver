...
TEST(ProgramTest, UnqualifiedTypeLookup) {
    ...
    TypeId type1Id = prog.findType(0, {"Type1"});
    TypeId type2Id = prog.findType(0, {"Type2"});
    ...
    TypeId nonExistentId = prog.findType(0, {"NonExistent"});
    EXPECT_FALSE(nonExistentId.defined());
    
    // Look up a global type from Mod1
    TypeId chanId = prog.findType(0, {"chan"});
...
TEST(ProgramTest, QualifiedTypeLookup) {
    ...
    TypeId type1Id = prog.findType(-1, {"Mod1", "Type1"});
    EXPECT_TRUE(type1Id.defined());
    EXPECT_EQ(type1Id.mod, 0);
    
    // Look up Mod2.Type3 from anywhere
    TypeId type3Id = prog.findType(-1, {"Mod2", "Type3"});
    EXPECT_TRUE(type3Id.defined());
    EXPECT_EQ(type3Id.mod, 1);
    
    // Look up a non-existent qualified name
    TypeId nonExistentId = prog.findType(-1, {"Mod1", "NonExistent"});
    EXPECT_FALSE(nonExistentId.defined());
    
    // Look up a non-existent module
    TypeId badModuleId = prog.findType(-1, {"NonExistentMod", "Type1"});
...
TEST(ProgramTest, TypeAccess) {
    ...
    // Look up a type
    TypeId typeId = prog.findType(0, {"Type1"});
    EXPECT_TRUE(typeId.defined());
